...
for i in range(len(tok1)):
    ...
    for j in range(len(tok1[i])):
        try:
            ...
            word_vector = np.random.normal(0,np.sqrt(0.25),300) 
...
embeddinWord2vectPretrained = docu_vectors

embeddinWord2vectPretrained = np.load("file.npy")
print(embeddinWord2vectPretrained.shape)

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(embeddinWord2vectPretrained, labels, test_size=0.20, random_state=seed)

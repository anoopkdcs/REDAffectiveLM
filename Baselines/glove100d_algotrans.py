...
#tok1=tok[44:45]
tok1=tok
docu_vectors=np.zeros((0,dim))
word_vec_array=np.zeros((0,dim))
doc_vector=np.zeros((0,dim),dtype=np.float64)
for i in range(len(tok1)):
    word_vector=np.zeros((0,dim),dtype=np.float64)
    for j in range(len(tok1[i])):
        try:
            word=tok1[i][j]
            word_vector = embeddings_dict[word]
            
        except KeyError:
            word_vector = np.random.normal(0,np.sqrt(0.25),dim) 
            
        word_vec_array=np.vstack((word_vec_array,word_vector))
    vecSum=word_vec_array.sum(axis=0)
    vecAvg=vecSum/len(tok1[i])
    docu_vectors=np.vstack((docu_vectors,vecAvg))
    print('Document----->'+str(i))
    
embeddinGloVePretrained = docu_vectors

np.save("file.npy",embeddinGloVePretrained)

embeddinGloVePretrained = np.load("file.npy")
print(embeddinGloVePretrained.shape)

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(embeddinGloVePretrained, labels, test_size=0.20, random_state=seed)
...
history = model.fit(x_train, y_train, validation_data=(x_val,y_val), epochs=epoc, batch_size=128, callbacks=[checkpointer], verbose=1)
model.summary()

...
NgramTestFeatures = x_val_vec.toarray()

x_test_vec = vectorizer.transform(x_test)
NgramTestFeatures = x_test_vec.toarray()

################################ ML models ######################
input_dimen = x_train_vec.shape[1]
model = Sequential()
model.add(Dense(128, input_dim=input_dimen, activation='relu', kernel_regularizer=l2(0.0001)))
...
history = model.fit(x_train_vec, y_train, validation_data=(x_val_vec,y_val), epochs=epoc, batch_size=64, callbacks=[checkpointer], verbose=1)
model.summary()
...
predict_test = best_model.predict(x_test_vec)

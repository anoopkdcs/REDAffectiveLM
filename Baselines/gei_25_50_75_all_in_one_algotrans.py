...
Path_intensityLex= "lexicon_file_name.tsv"
...
Graded_Emotion_Intensity_delta25 = np.zeros((dataDim,emoDIm))
delta = 0.75

'''
Test with muliple word occurance
t = ['hussein']
tokenised[1] =tokenised[1] +t 
'''
...
for dc in range(len(tokenised)):
    ...
    for wrd in wordCounts:
        ...
        if tmpWord in DepchEmolexDic:
            ...
            tempflotEmovector = np.reshape(np.asarray(list(np.float_(tempEmovector))), (1,5))
            tempflotEmovector[tempflotEmovector < delta] = 0.0
            tmpWordcount = wordCounts[tmpWord]
            updations = tempflotEmovector*tmpWordcount
            Graded_Emotion_Intensity_delta25[dc] = Graded_Emotion_Intensity_delta25[dc] + updations

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(Graded_Emotion_Intensity_delta25, labels, test_size=0.20, random_state=seed)

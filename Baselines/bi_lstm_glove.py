...
nanLoc = np.argwhere(np.isnan(labels))
print(nanLoc)
...
max_features = 39088 #27749 #27749 #120305 #13260  how many unique words to use (i.e num rows in embedding vector)
...
#Find the location of any NAN entry and replace with 0.0
nanLoc = np.argwhere(np.isnan(x_test_padded))
print(nanLoc)

type(x_test_padded)
np.where(~x_val_padded.any(axis=1))[0]
...
#Find the location of any NAN entry and replace with 0.0
nanLoc = np.argwhere(np.isnan(embedding_matrix))
print(nanLoc)

np.where(~embedding_matrix.any(axis=1))[0]
...
X = Bidirectional(LSTM(100, return_sequences=False, 
         activation='relu', 
...
print(predict_test[607])
print(y_test[607])
print(x_test[607])

print(predict_test[1:10])
print(predict_test[2000])
print(predict_test[100])
print(predict_test[1000])

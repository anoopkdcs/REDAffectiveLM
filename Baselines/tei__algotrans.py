...
Path_intensityLex= "DepecheMood_english_token_full.tsv"
...
Total_Emotion_Intensity = np.zeros((dataDim,emoDIm))
for dc in range(len(tokenised)):
    ...
    for wrd in wordCounts:
        ...
        if tmpWord in DepchEmolexDic:
            ...
            tmpNPvector = np.array(tempflotEmovector)    
            updations = tmpNPvector*tmpWordcount
            Total_Emotion_Intensity[dc] = Total_Emotion_Intensity[dc] + updations

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(Total_Emotion_Intensity, labels, test_size=0.20, random_state=seed)
...
history = model.fit(x_train, y_train, validation_data=(x_val,y_val), epochs=epoc, batch_size=32, callbacks=[checkpointer], verbose=1)
model.summary()

...
dim=100
docs2 = [TaggedDocument(words=word_tokenize(_d.lower()), tags=[str(i)]) for i, _d in enumerate(deTokenized)]
model = gensim.models.Doc2Vec(docs2, dm = 1, alpha=0.025, size= dim, min_alpha=0.025, min_count=0)

docu_vectors=np.zeros((0,dim))
doc_vec=np.zeros((0,dim))
for i in range(len(tokenised)):
    doc_vec=model.docvecs[i]
    print (i)
    docu_vectors=np.vstack((docu_vectors,doc_vec))

embeddinDoc2vec = docu_vectors

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(embeddinDoc2vec, labels, test_size=0.20, random_state=seed)
...
model = Sequential()
model.add(Dense(128, input_dim=input_dimen, activation='relu', kernel_regularizer=l2(0.01)))

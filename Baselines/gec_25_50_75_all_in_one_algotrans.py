...
GradedEmotionCount_delta25 = np.zeros((dataDim,emoDIm))
delta = 0.75
...
for dc in range(len(tokenised)):
    ...
    for wrd in wordCounts:
        ...
        if tmpWord in DepchEmolexDic:
            ...
            noZeroCount = np.count_nonzero(tempflotEmovector)
            if noZeroCount != 0:
                maxIndex = np.argmax(tempflotEmovector)
                tmpWordcount = wordCounts[tmpWord]
                updations = 1*tmpWordcount
                GradedEmotionCount_delta25 [dc][maxIndex] = GradedEmotionCount_delta25 [dc][maxIndex] + updations

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(GradedEmotionCount_delta25, labels, test_size=0.20, random_state=seed)

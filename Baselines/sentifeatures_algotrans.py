...
''' Feature vector Creation
1. Document polarity (+,-) intensity : Find the average intensity; both + and - of the document
2. Document polarity (+,-) count : # of + and - words in the document
'''
#Document polarity (+,-) intensity
intensitySA = [] #pos, neu, neg 
analyzer = SentimentIntensityAnalyzer()
for j in range(len(deTokenized)):
    sa = analyzer.polarity_scores(deTokenized[j])
    tempPos = sa['pos']
    tempNeu = sa['neu']
    temNeg = sa['neg']
    tempfVector = [tempPos, tempNeu ,temNeg]
    intensitySA.append(tempfVector)

#Document polarity (+,-)
countSA = [] #pos, neu, neg     
for k in range(len(tokenised)):
    tempPoscount = 0
    tempNeucont = 0
    tempNegcount = 0
    for m in range(len(tokenised[k])):
        sc = analyzer.polarity_scores(tokenised[k][m])
        scpos = sc['pos']
        scneu = sc['neu']
        scneg = sc['neg']
        if scpos > 0:
            tempPoscount = tempPoscount + 1
        if scneu > 0:
             tempNeucont = tempNeucont + 1
        if scneg > 0:
            tempNegcount = tempNegcount + 1
    tempCountVector = [tempPoscount, tempNeucont,tempNegcount]
    countSA.append(tempCountVector)

#Converting Vectors into array
intArray = np.asarray(intensitySA)
countArray = np.asarray(countSA)
SAfeatureVector = np.concatenate((intensitySA,countArray), axis = 1)

################################ train test val split ######################
x_train_val, x_test, y_train_val, y_test = train_test_split(SAfeatureVector, labels, test_size=0.20, random_state=seed)

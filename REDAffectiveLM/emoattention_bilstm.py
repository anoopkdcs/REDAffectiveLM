...
x_train_val_original, x_test_original, y_train_val_original, y_test_original = train_test_split(headlines, labels, test_size=0.20, random_state=seed)
x_train_original, x_val_original, y_train_original, y_val_original = train_test_split(x_train_val_original, y_train_val_original, test_size=0.25, random_state=seed)
...
embeddings_index = {}
f = open(os.path.join('path', 'em-glove.6B.300d-20epoch.txt'))
for line in f:
    values = line.split()
    word = values[0]
...
X = Bidirectional(LSTM(300, return_sequences=True, 
         activation='relu', 
...
checkpointer = ModelCheckpoint(filepath='file_name.h5', verbose=1, save_best_only=True)
...
####################### Evaluation1: RMSE ######################
del model
best_model = load_model('file_name.h5', custom_objects={'Attention': Attention})
...
modelVis.load_weights('file_name.h5')
...
def visualize_attention(Sid):
    ...
    print("Original Emotion Label:"+ str(y_test[idx]))
    
    print("Original News Snippet:\n" + x_test_original[idx])
    print("Original Emotion Label:"+ str(y_test_original[idx]))
